...
class Panel extends Model{
     ...
     * @var array
     */
    protected $appends = ['dev','test','expect_finish_date_format','actual_finish_date_format'];
...
//region 自定义属性
     ...
     * @return String
     */
    public function getExpectFinishDateFormatAttribute()
    {
        return $this->attributes['expect_finish_date_format'] ="2016-03-09 00:00:00";
    }
     ...
     * @return String
     */
    public function getActualFinishDateFormatAttribute()
    {
        return $this->attributes['actual_finish_date_format'] =$this->getActualFinishDate();
    }
...
//endregion
    ...
    protected function getActualFinishDate()
    {
        if(date_default_timezone_get() != "prc") date_default_timezone_set("prc");
        //strtotime: 将任何英文文本的日期时间描述解析为 UNIX 时间戳
        $ori_time=date('y-m-d',strtotime($this->actual_finish_date));
        $now_time=date('y-m-d',time());

        $day = floor((strtotime($ori_time)-strtotime($now_time))/86400);
        if($day==0){
            return "今天";
        }elseif($day=1){
            return "昨天";
